...
- **Stream API** (`node:stream` layers over the classes above; upstream `lib/producer-stream.js` / `lib/kafka-consumer-stream.js`):
  - `Producer.createWriteStream(conf, topicConf, streamOptions)` / module-level `createWriteStream(...)` → `ProducerStream extends Writable` with a public `producer` field, `connect()`, `close(cb?)`. Options (`WriteStreamOptions`): `topic` (required unless `objectMode`), `objectMode` (chunks are `{topic, partition, value, key, timestamp, opaque, headers}` instead of buffers), `encoding`, `autoClose` (default `true`: flush + disconnect the producer on `finish`/error), `pollInterval` (default 1000 → `setPollInterval`), `connectOptions`, plus any `WritableOptions` (`highWaterMark`, …). Auto-connects the producer; writes queue until `ready`; `ERR__QUEUE_FULL` is retried with backoff (backpressure), other errors fail the write.
  - `KafkaConsumer.createReadStream(conf, topicConf, streamOptions | waitInterval)` / module-level `createReadStream(...)` → `ConsumerStream extends Readable` (upstream class name `KafkaConsumerStream`) with a public `consumer` field, `connect()`, `close(cb?)`. Options (`ReadStreamOptions`): `topics` (string | RegExp | array | `(metadata) => topics`), `waitInterval` (default 1000 ms, randomized retry delay when idle; `0` → immediate), `fetchSize` (default 1; messages per `consume(n)`), `objectMode` (default `true`; `false` pushes `message.value` buffers), `highWaterMark` (default `fetchSize + 2` in objectMode), `autoClose` (default `true`: unsubscribe + disconnect on `end`/`destroy`), `streamAsBatch` (push each fetched batch as one array), `connectOptions`, plus any `ReadableOptions`. Consume errors are emitted as `error` and consumption continues; consumer `unsubscribed` ends the stream.
...
- **`Consumer`** — `connect()`, `disconnect()`, `subscribe({topics, replace?})`, `run({eachMessage?, eachBatch?, partitionsConsumedConcurrently?})`, `commitOffsets()`, `committed()`, `storeOffsets()`, `seek()`, `pause()` (returns a resume function)/`resume()`/`paused()`, `assignment()`, `stop()`, `logger()`, `dependentAdmin()`; `rebalance_cb(err, assignment, assignmentFns)` with upstream semantics; `_getInternalClient()` for `producer.sendOffsets({consumer})`.
